...
class TRSGrammar(object):
    ...
    def sample_trs(self, p_rule, invent=True):
        """
        sample a TRS
    ...
    def log_p_trs(self, trs, p_rule, invent=True):
        """
        ...
        float
            log(p(trs | p_rule))
        """
        p = ss.geom.logpmf(trs.num_rules(), p=p_rule)
        for rule in trs.rules():
            self.variables = set()
            p += self.log_p_rule(rule, invent=invent)
        return p
